    ...
    methods (Static)
        function report = assessSignals(time, rawLatG, speedMps, yawRateRadps, steerRad, wheelbase)
            ...
            [time, rawLatG, speedMps, yawRateRadps, steerRad] = ...
                lts.diagnostics.LateralGDiagnostics.trimToCommonLength( ...
                time, rawLatG, speedMps, yawRateRadps, steerRad);

            yawLatG = speedMps .* yawRateRadps ./ lts.diagnostics.LateralGDiagnostics.g;
            steerLatG = speedMps.^2 .* tan(steerRad) ./ ...
                wheelbase ./ lts.diagnostics.LateralGDiagnostics.g;
            ...
            report.rawPeakAbsG = lts.diagnostics.LateralGDiagnostics.maxAbs(rawLatG);
            report.rawP95AbsG = lts.diagnostics.LateralGDiagnostics.percentileAbs(rawLatG, 95);
            report.yawPeakAbsG = lts.diagnostics.LateralGDiagnostics.maxAbs(yawLatG);
            report.yawP95AbsG = lts.diagnostics.LateralGDiagnostics.percentileAbs(yawLatG, 95);
            report.steerPeakAbsG = lts.diagnostics.LateralGDiagnostics.maxAbs(steerLatG);
            report.steerP95AbsG = lts.diagnostics.LateralGDiagnostics.percentileAbs(steerLatG, 95);
            report.signMismatchFraction = ...
                lts.diagnostics.LateralGDiagnostics.safeFraction(nnz(signMismatch), nnz(activeYaw));
            report.magnitudeMismatchFraction = ...
                lts.diagnostics.LateralGDiagnostics.safeFraction(nnz(magnitudeMismatch), nnz(validYaw));
            report.worstYawErrorG = lts.diagnostics.LateralGDiagnostics.maxAbs(rawLatG - yawLatG);
            report.worstEvent = lts.diagnostics.LateralGDiagnostics.worstEvent( ...
                time, rawLatG, yawLatG, steerLatG, speedMps);
            report.messages = lts.diagnostics.LateralGDiagnostics.consistencyMessages(report);
        end
        ...
        function events = topMismatchEvents(time, rawLatG, yawLatG, steerLatG, speedMps, count, minSeparationSec)
            ...
            events = repmat(lts.diagnostics.LateralGDiagnostics.emptyEvent(), 0, 1);
            for idx = 1:count
                [bestScore, bestIdx] = max(score);
                if ~isfinite(bestScore) || bestScore < 0
                    break;
                end
                events(end + 1, 1) = lts.diagnostics.LateralGDiagnostics.eventAt( ...
    ...
    methods (Static, Access = private)
        ...
        function event = worstEvent(time, rawLatG, yawLatG, steerLatG, speedMps)
            ...
            if isempty(idx) || ~isfinite(bestScore) || bestScore < 0
                event = lts.diagnostics.LateralGDiagnostics.emptyEvent();
                return;
            end
            event = lts.diagnostics.LateralGDiagnostics.eventAt( ...
