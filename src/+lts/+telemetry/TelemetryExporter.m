classdef TelemetryExporter
    ...
    methods (Static)
        function filepath = writeToMoTeCFormat(stateLog, filepath, varargin)
            ...
            %   filepath = lts.telemetry.TelemetryExporter.writeToMoTeCFormat(stateLog, filepath)
            ...
            lts.telemetry.TelemetryExporter.validateStateLog(stateLog);
            filepath = lts.telemetry.TelemetryExporter.ensureCsvPath(filepath);
            ...
            tableData = lts.telemetry.TelemetryExporter.buildMoTeCTable(stateLog, channels, includeDerived);
            lts.telemetry.TelemetryExporter.writeCsv(filepath, tableData);
        ...
        function [csvFile, ldFile] = exportToMoTeCLog(stateLog, csvFile, varargin)
            ...
            %   [csvFile, ldFile] = lts.telemetry.TelemetryExporter.exportToMoTeCLog(stateLog, csvFile)
            ...
            csvFile = lts.telemetry.TelemetryExporter.writeToMoTeCFormat( ...
                stateLog, csvFile, ...
                'Channels', parser.Results.Channels, ...
                'IncludeDerived', parser.Results.IncludeDerived);

            convertArgs = lts.telemetry.TelemetryExporter.nameValueStructToCell(parser.Unmatched);
            ldFile = lts.telemetry.TelemetryExporter.convertCsvToMoTeCLog(csvFile, convertArgs{:});
        end
        ...
        function ldFile = convertCsvToMoTeCLog(csvFile, varargin)
            ...
            if ~exist(csvFile, 'file')
                error('lts_telemetry_TelemetryExporter:MissingCsv', ...
            ...
            if isempty(generatorPath)
                generatorPath = lts.telemetry.TelemetryExporter.defaultGeneratorPath();
            end

            if ~exist(generatorPath, 'file')
                error('lts_telemetry_TelemetryExporter:MissingGenerator', ...
                    ['MoTeC log generator was not found at "%s". ' ...
                    'Run "git submodule update --init --recursive".'], generatorPath);
            end

            args = { ...
                lts.util.shellQuote(generatorPath), ...
                lts.util.shellQuote(csvFile), ...
                'CSV', ...
                '--output', lts.util.shellQuote(ldFile)};
            ...
            command = strjoin([{lts.util.shellQuote(parser.Results.PythonCommand)} args], ' ');
    ...
    methods (Static, Access = private)
        function validateStateLog(stateLog)
            ...
            if ~isfield(stateLog, 'time') || isempty(stateLog.time)
                error('lts_telemetry_TelemetryExporter:MissingTime', ...
        ...
        function tableData = buildMoTeCTable(stateLog, requestedChannels, includeDerived)
            ...
            if includeDerived
                ...
                tableData = lts.telemetry.TelemetryExporter.addRawChannel(tableData, stateLog, 'time', 'Time', 's', nSamples);
            end

            if isempty(requestedChannels)
                requestedChannels = lts.telemetry.TelemetryExporter.defaultChannelOrder(stateLog);
            end

            for i = 1:numel(requestedChannels)
                field = requestedChannels{i};
                if ~isfield(stateLog, field)
                    warning('lts_telemetry_TelemetryExporter:MissingChannel', ...
                        'Skipping missing telemetry channel "%s".', field);
                    continue;
                end

                if lts.telemetry.TelemetryExporter.hasChannel(tableData, field)
                    continue;
                end
        ...
        function tableData = addDerivedChannels(tableData, stateLog, nSamples)
            ...
            if isfield(stateLog, 'speed')
                tableData = lts.telemetry.TelemetryExporter.addRawChannel(tableData, stateLog, 'speed', 'Speed mps', 'm/s', nSamples);
            end

            if isfield(stateLog, 'motorRPM')
                tableData = lts.telemetry.TelemetryExporter.addComputedChannel(tableData, 'motorRPM', ...
                    'Engine RPM', stateLog.motorRPM(:), 'rpm', nSamples);
                tableData = lts.telemetry.TelemetryExporter.addComputedChannel(tableData, 'motorRPMAlias', ...
                    'Motor RPM', stateLog.motorRPM(:), 'rpm', nSamples);
            end

            if isfield(stateLog, 'ax')
                tableData = lts.telemetry.TelemetryExporter.addComputedChannel(tableData, 'axG', 'G Sensor Front Acceleration Longitudinal', ...
                    stateLog.ax(:) / 9.81, 'G', nSamples);
            end

            if isfield(stateLog, 'ay')
                tableData = lts.telemetry.TelemetryExporter.addComputedChannel(tableData, 'ayG', 'G Sensor Front Acceleration Lateral', ...
                    stateLog.ay(:) / 9.81, 'G', nSamples);
            end

            if isfield(stateLog, 'frontAxleAy')
                tableData = lts.telemetry.TelemetryExporter.addComputedChannel(tableData, 'frontAxleAyG', ...
                    'G Sensor Front Axle Acceleration Lateral', ...
                    stateLog.frontAxleAy(:) / 9.81, 'G', nSamples);
            end

            if isfield(stateLog, 'rearAxleAy')
                tableData = lts.telemetry.TelemetryExporter.addComputedChannel(tableData, 'rearAxleAyG', ...
            ...
            pctChannels = { ...
                ...
                'brake', 'Brake'; ...
                'replayBrake', 'Replay Brake Input'; ...
                ...
                'tireUtilization_FL', 'Tire Utilization FL'; ...
                'tireUtilization_FR', 'Tire Utilization FR'; ...
                'tireUtilization_RL', 'Tire Utilization RL'; ...
                'tireUtilization_RR', 'Tire Utilization RR'};
            for i = 1:size(pctChannels, 1)
                field = pctChannels{i, 1};
                if isfield(stateLog, field)
                    tableData = lts.telemetry.TelemetryExporter.addComputedChannel(tableData, field, pctChannels{i, 2}, ...
                        stateLog.(field)(:) * 100, '%', nSamples);
                end
            end

            barChannels = { ...
                'brakePressureFrontBar', 'Brake Pressure Front'; ...
                'brakePressureRearBar', 'Brake Pressure Rear'; ...
                'replayBrakePressureFrontBar', 'Replay Brake Pressure Front'; ...
                'replayBrakePressureRearBar', 'Replay Brake Pressure Rear'};
            for i = 1:size(barChannels, 1)
                field = barChannels{i, 1};
                if isfield(stateLog, field)
                    tableData = lts.telemetry.TelemetryExporter.addRawChannel( ...
                        tableData, stateLog, field, barChannels{i, 2}, 'bar', nSamples);
                end
            end

            degChannels = { ...
                'steer', 'Steer'; ...
                'replaySteer', 'Replay Steer Input'; ...
                'heading', 'Heading'; ...
                'bodySlipAngle', 'Body Slip Angle'; ...
                'pitchAngle', 'Pitch'; ...
                'rollAngle', 'Roll'; ...
                'frontRollAngle', 'Roll Front'; ...
                'rearRollAngle', 'Roll Rear'; ...
                'twistAngle', 'Chassis Twist'; ...
                'camber_FL', 'Camber FL'; ...
                'camber_FR', 'Camber FR'; ...
                'camber_RL', 'Camber RL'; ...
                'camber_RR', 'Camber RR'; ...
                ...
                'slipAngle_FL', 'Slip Angle FL'; ...
                'slipAngle_FR', 'Slip Angle FR'; ...
                'slipAngle_RL', 'Slip Angle RL'; ...
                'slipAngle_RR', 'Slip Angle RR'};
            ...
            for i = 1:size(mmChannels, 1)
                ...
                if isfield(stateLog, field)
                    tableData = lts.telemetry.TelemetryExporter.addComputedChannel(tableData, field, mmChannels{i, 2}, ...
            ...
            for i = 1:size(mmPerSecChannels, 1)
                ...
                if isfield(stateLog, field)
                    tableData = lts.telemetry.TelemetryExporter.addComputedChannel(tableData, field, mmPerSecChannels{i, 2}, ...
            ...
            for i = 1:size(omegaChannels, 1)
                ...
                if isfield(stateLog, field)
                    tableData = lts.telemetry.TelemetryExporter.addComputedChannel(tableData, [field 'RPM'], omegaChannels{i, 2}, ...
        ...
        function tableData = addFakeGpsChannels(tableData, stateLog, nSamples)
            ...
            [latitude, longitude] = lts.telemetry.TelemetryExporter.fakeGpsFromPosition(stateLog, nSamples);
            tableData = lts.telemetry.TelemetryExporter.addComputedChannel(tableData, 'gpsLatitude', ...
                'GPS Latitude', latitude, 'deg', nSamples);
            tableData = lts.telemetry.TelemetryExporter.addComputedChannel(tableData, 'gpsLongitude', ...
        ...
        function tableData = addComputedChannel(tableData, field, channelName, values, unit, nSamples)
            if numel(values) ~= nSamples
                warning('lts_telemetry_TelemetryExporter:ChannelLengthMismatch', ...
        ...
        function writeCsv(filepath, tableData)
            ...
            if fid < 0
                error('lts_telemetry_TelemetryExporter:FileOpenFailed', ...
        ...
        function row = csvRow(values)
            cells = cell(1, numel(values));
            for i = 1:numel(values)
                cells{i} = lts.telemetry.TelemetryExporter.csvEscape(values{i});
            end
            row = strjoin(cells, ',');
        end
        ...
        function args = addOptionalCliArg(args, flag, value)
            ...
            args(end + 1:end + 2) = {flag, lts.util.shellQuote(value)};
        end
