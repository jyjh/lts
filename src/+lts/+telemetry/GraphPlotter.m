classdef GraphPlotter
    ...
    methods(Static)
        ...
        function plotAll(stateLog, lapTime, track, vehicle, aero, singleWindow)
            ...
            lts.telemetry.GraphPlotter.plotDriver(stateLog, singleWindow, 5);
            lts.telemetry.GraphPlotter.plotAero(stateLog, vehicle, aero, singleWindow, 9);
            lts.telemetry.GraphPlotter.plotSuspension(stateLog, vehicle, singleWindow, 13);
            lts.telemetry.GraphPlotter.plotTire(stateLog, vehicle, singleWindow, 17);
            if ~singleWindow
                lts.telemetry.GraphPlotter.plotWheelSpeeds(stateLog, vehicle);
            end
            lts.telemetry.GraphPlotter.plotPowertrain(stateLog, vehicle, singleWindow, 21);
        end
        ...
        function plotAero(stateLog, vehicle, aero, useSingleFigure, startIdx)
            ...
            for j = 1:numel(sampleSpeeds)
                tempState = lts.simulation.VehicleState('speed', sampleSpeeds(j), 'ax', 0, 'pitchAngle', 0, 'rideHeight', 0);
