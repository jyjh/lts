classdef CorrelationTrackAlignment
    ...
    methods (Static)
        function [trackOut, info] = align(track, profile, varargin)
                ...
                if strcmp(mode, 'auto')
                    if profile.hasGpsCourse()
                        [station, estimateErrorDeg, sampleCount] = ...
                            CorrelationTrackAlignment.estimateStartStationFromCourse( ...
                                track, profile, ...
                        ...
                        warning('CorrelationTrackAlignment:MissingCourse', ...
                    ...
                    if ~isfinite(station)
                        error('CorrelationTrackAlignment:InvalidStartStation', ...
            ...
            station = CorrelationTrackAlignment.wrapStation(station, trackLength);
            info.startStationM = station;

            trackOut = track;
            if abs(station) > 1e-9
                trackOut = CorrelationTrackAlignment.rebaseTrack(track, station);
                info.rebased = true;
            end

            info.headingErrorDeg = CorrelationTrackAlignment.initialHeadingErrorDeg( ...
                trackOut, profile);
        ...
        function [station, errorDeg, sampleCount] = estimateStartStationFromCourse( ...
            ...
            if nnz(keep) < 3
                error('CorrelationTrackAlignment:InsufficientCourse', ...
            ...
            loggedCurvature = CorrelationTrackAlignment.loggedCurvatureSignals( ...
                profile, keep);

            [trackS, trackHeading, trackLength] = ...
                CorrelationTrackAlignment.trackHeadingSamples(track);
            [trackCurvS, trackCurvature, ~] = ...
                CorrelationTrackAlignment.trackCurvatureSamples(track);
            ...
            for i = 1:numel(candidates)
                ...
                modelHeading = CorrelationTrackAlignment.headingAtStation( ...
                    trackS, trackHeading, trackLength, ...
                    queryStation);
                err = CorrelationTrackAlignment.angularDiff(modelHeading, queryCourse);
                ...
                if ~isempty(finiteErr)
                    ...
                    if firstErrors(i) <= initialHeadingTolerance
                        curvatureCost = CorrelationTrackAlignment.curvatureCost( ...
            ...
            if all(~isfinite(scores))
                warning('CorrelationTrackAlignment:NoInitialHeadingMatch', ...
        ...
        function trackOut = rebaseTrack(track, startStationM)
            ...
            closed = CorrelationTrackAlignment.isClosedTrack(track, points);
            if ~closed
                if abs(startStationM) <= 1e-9
                    trackOut = track;
                    return;
                end
                error('CorrelationTrackAlignment:OpenTrackRebase', ...
                    'Only closed tracks can be rebased to a nonzero start station.');
            end

            trackLength = track.getTotalLength();
            startStationM = CorrelationTrackAlignment.wrapStation(startStationM, trackLength);
            if abs(startStationM) <= 1e-9
                trackOut = track;
                return;
            end
            ...
            metadata.correlation_alignment_source = 'CorrelationTrackAlignment';
            ...
            trackOut = components.WaypointTrack(rebasedPoints, ...
        ...
        function errorDeg = initialHeadingErrorDeg(track, profile)
            ...
            errorRad = CorrelationTrackAlignment.angularDiff(heading(1), course(1));
        ...
        function [trackS, trackHeading, trackLength] = trackHeadingSamples(track)
            points = track.getTrackPoints();
            closed = CorrelationTrackAlignment.isClosedTrack(track, points);
            trackS = components.Track.cumulativeArcLength(points, closed);
        ...
        function cost = curvatureCost(trackS, trackCurvature, trackLength, queryS, loggedCurvature)
            ...
            for i = 1:numel(loggedCurvature)
                ...
                eSame = CorrelationTrackAlignment.meanFinite(abs(modelCurvature(:) - logged(:)));
                eFlipped = CorrelationTrackAlignment.meanFinite(abs(modelCurvature(:) + logged(:)));
                costs(i) = min(eSame, eFlipped);
            end
            cost = CorrelationTrackAlignment.meanFinite(costs);
        end
        ...
        function station = wrapStation(station, trackLength)
            if ~isfinite(station)
                error('CorrelationTrackAlignment:InvalidStation', ...
